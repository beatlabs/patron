...
├── metric.go    # Cache operation metrics (hit/miss/evict)
...
|--------|------|----------|------|
| `Cache` | interface | `cache.go:10` | `Get(ctx, key)`, `Set(ctx, key, val)`, `Remove(ctx, key)`, `Purge(ctx)` — values are `any` |
| `TTLCache` | interface | `cache.go:22` | Extends `Cache` + `SetTTL(ctx, key, val, ttl)` |
| `lru.Cache` | struct | `lru/lru.go:20` | Generic LRU wrapping hashicorp/golang-lru with metrics |
| `lru.New` | func | `lru/lru.go:27` | `New[k, v](size, useCase)` — creates LRU cache |
| `lru.NewWithEvict` | func | `lru/lru.go:38` | `NewWithEvict(size, useCase, onEvict)` — LRU with eviction callback |
...
- The `Cache`/`TTLCache` interfaces are non-generic (values are `any`); the LRU implementation is generic over key and value types (`lru.Cache[k comparable, v any]`).
- Redis implementation requires integration tests (`//go:build integration`).

## Adding a new implementation

1. Create `cache/<name>/<name>.go`
2. Implement `Cache` or `TTLCache` interface
