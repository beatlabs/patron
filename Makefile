...
lint: fmtcheck
	$(DOCKER) run --env=GOFLAGS=-mod=vendor --rm -v $(CURDIR):/app -w /app golangci/golangci-lint:v1.49.0 golangci-lint -v run

deeplint: fmtcheck
	$(DOCKER) run --env=GOFLAGS=-mod=vendor --rm -v $(CURDIR):/app -w /app golangci/golangci-lint:v1.49.0 golangci-lint run --exclude-use-default=false --enable-all -D dupl --build-tags integration
